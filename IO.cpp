...
//Terrible name, but DrawText was already in use
void IO::DrawSomeText (char * text, int posX, int posY)
{
	TTF_Font *fntAgent = TTF_OpenFont( "Fonts\\5thagent.ttf", 12 );
		SDL_Color clrFg = {255,255,255,0};
		SDL_Surface *sText = TTF_RenderText_Solid( fntAgent, text, clrFg );
		SDL_Rect rcDest = {posX,posY,0,0};
		SDL_BlitSurface( sText,NULL, mScreen,&rcDest );
	SDL_FreeSurface( sText );

TTF_CloseFont( fntAgent );
}
