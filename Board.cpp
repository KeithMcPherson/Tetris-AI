...
returns the score to increase by
====================================== 
*/
...
vector<double> Board::GetBoardNNInputs()
{
	vector<double> board;
	for (int i=0; i < BOARD_WIDTH; ++i)
		for (int j=0; j< BOARD_HEIGHT; ++j)
...
//Calculates the weight for the board based on the AI's scoring criteria (should probably be part of AI)
//I'm sure there's a better way to do this, at the very least there's a more efficient way.
int Board::GetBoardWeight()
{
	mEdges = 0;
	for (int i=0; i<BOARD_WIDTH; i++)
		for (int j=0; j<BOARD_HEIGHT; j++)
			{
				...
				if(mBoard[i][j]==POS_FREE){
					int tempScoring=0;
						for (int k=j; k>0;k--)
							if(mBoard[i][k]==POS_FILLED)
								tempScoring+=1;
					if(tempScoring>0)
						mEdges+=1;
				}
			#endif
			#ifndef SIMPLE_SCORING
				//Top left corner
				if(i==0 && j==0){
					if (mBoard[i][j]==POS_FILLED){
						if (mBoard[i+1][j]==POS_FREE)
							mEdges+=1+(BOARD_HEIGHT-j)/3;
						if (mBoard[i][j+1]==POS_FREE)
							mEdges+=1+(BOARD_HEIGHT-j)/3;
					}
					else
						mEdges+=1+(BOARD_HEIGHT-j)/3;
				...
				//right board edge
				if(i==BOARD_WIDTH-1 && j>0 && j<BOARD_HEIGHT-1){
					if (mBoard[i][j]==POS_FILLED){
						if (mBoard[i-1][j]==POS_FREE)
							mEdges+=1+(BOARD_HEIGHT-j)/3;
						if (mBoard[i][j-1]==POS_FREE)
				...
				//bottom board edge
				if(j==BOARD_HEIGHT-1 && i>0 && i<BOARD_WIDTH-1){
					if (mBoard[i][j]==POS_FILLED){
						if (mBoard[i][j-1]==POS_FREE)
							mEdges+=1+(BOARD_HEIGHT-j)/3;
						if (mBoard[i-1][j]==POS_FREE)
				...
				//Current block is not an edge or a corner
				if(i!=0 && i!=BOARD_WIDTH-1 && j!=0 && j!=BOARD_HEIGHT-1 && mBoard[i][j]==POS_FILLED){
					if (mBoard[i-1][j]==POS_FREE)
						mEdges+=1+(BOARD_HEIGHT-j)/3;
					if (mBoard[i+1][j]==POS_FREE)
						mEdges+=1+(BOARD_HEIGHT-j)/3;
				...
				//This counteracts placing pieces that would end the game
				if(j<=4 && mBoard[i][j] == POS_FILLED)
					mEdges+=5;
			#endif
			}
		return mEdges;
